...
func Get() Command {
	...
	if (*globals.JSONFilePath != "" || *globals.CSVFilePath != "") && (*globals.Type == "reminder") {
		return &uploadRemindersFromCSVCommand{}
	}
			...
			for e := range recordStream {
				...
				default:
					...
					if len(dataSlice) == ctBatchSize {
						...
						p["d"] = dataSlice
						if *globals.Type == "reminder" {
							sendDataToCTAPI(p, "https://"+region+reminderUploadEndpoint)
						} else {
